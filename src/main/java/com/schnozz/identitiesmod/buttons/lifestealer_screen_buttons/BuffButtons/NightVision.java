...
public class NightVision extends Button{
    ...
    public void onPress()
    {
        ...
        if (p.getMaxHealth() >= 20 + cost && permLevel < 0)
        {
            ...
            p.getData(ModDataAttachments.LIFESTEALER_BUFFS).setLifestealerBuff(ResourceLocation.fromNamespaceAndPath(IdentitiesMod.MODID,"night_vision"),permLevel+1);

            PacketDistributor.sendToServer(new PotionLevelPayload(MobEffects.NIGHT_VISION, (int)permLevel, MobEffectInstance.INFINITE_DURATION));
        }
    }
}
